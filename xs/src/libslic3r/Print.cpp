...
Print::_make_brim()
{
    ...
    if (this->config.brim_connections_width > 0) {
        ...
        InfillPattern fill_pattern = layer_region->getInfillPattern();

        if(fill_pattern == ipBioRectilinear){
          const double fill_spacing = layer_region->getInfillSpacing();
          filler->min_spacing  = fill_spacing;
          filler->fill_spacing = fill_spacing;
        }else{
